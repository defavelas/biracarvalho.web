...
use App\Adapter\Kobo\AccessibleLocationAdapter;
...
final class AccessibleLocationsProcess
{
    public function __construct(
        private readonly AccessibleLocationsService $accessibleLocationsService,
        private readonly AccessibleLocationAdapter $adapter,
    ) {}
    ...
    {
        Log::info('Starting accessible locations processing');
    ...
    private function updateLocation(Location $location, array $data): string
    {
        return DB::transaction(function () use ($location, $data) {
            ...
            Log::info('Updated existing location', [
                'id' => $location->id,
                'external_id' => $location->external_id,
                'name' => $location->name,
            ]);
    ...
    private function createInfos(Location $location, array $infosData): void
    {
        ...
        Log::debug('Created infos for location', [
            'location_id' => $location->id,
